...
class AiService
{
    ...
    // 会员等级定义（硬编码兜底，优先从远程API/推送配置获取）
    private static $tiers = [
        'free'       => ['name' => '免费版', 'limit' => 100000,  'price' => 0,   'price_yearly' => 0],
        'basic'      => ['name' => '基础版', 'limit' => 1000000, 'price' => 99,  'price_yearly' => 990],
        'pro'        => ['name' => '专业版', 'limit' => 5000000, 'price' => 299, 'price_yearly' => 2990],
        'enterprise' => ['name' => '企业版', 'limit' => 50000000,'price' => 999, 'price_yearly' => 9990],
    ];
    private static $remoteTiersCache = null;
    ...
    {
        $info = self::getActivationInfo($userId);
        if (!$info || !isset($info['expiry'])) return false;
        if (time() > intval($info['expiry'])) return false;
        return true;
    }

    public static function getActivationInfo($userId = null)
    {
        $uid = $userId ?: 0;
        $file = app()->getRootPath() . 'runtime' . DIRECTORY_SEPARATOR . 'ai_license_' . $uid . '.dat';
        if (!file_exists($file)) return null;
        return json_decode(file_get_contents($file), true);
    }

    public static function saveActivation($info, $userId = null)
    {
        $uid = $userId ?: ($info['user_id'] ?? 0);
        $file = app()->getRootPath() . 'runtime' . DIRECTORY_SEPARATOR . 'ai_license_' . $uid . '.dat';
        $dir = dirname($file);
        if (!is_dir($dir)) mkdir($dir, 0755, true);
    ...
    private static function usageFile($userId = null)
    {
        $uid = $userId ?: 0;
        return app()->getRootPath() . 'runtime' . DIRECTORY_SEPARATOR . 'ai_usage_' . $uid . '.json';
    }

    public static function getUsageData($userId = null)
    {
        $month = date('Y-m');
        $file = self::usageFile($userId);
        $data = file_exists($file) ? json_decode(file_get_contents($file), true) : [];
        if (!is_array($data)) $data = [];
        return $data[$month] ?? ['input_tokens' => 0, 'output_tokens' => 0, 'total_tokens' => 0];
    }

    public static function trackUsage($inputTokens, $outputTokens, $userId = null)
    {
        $month = date('Y-m');
        $file = self::usageFile($userId);
        $data = file_exists($file) ? json_decode(file_get_contents($file), true) : [];
     ...
     */
    public static function getQuotaInfo($userId = null)
    {
        $info = self::getActivationInfo($userId ?? null);
        $tier = $info['tier'] ?? 'free';
        $limit = self::$tiers[$tier]['limit'] ?? 100000;
        $usage = self::getUsageData($userId);
     ...
     * 检查是否有剩余配额
     */
    public static function checkQuota($userId = null)
    {
        $quota = self::getQuotaInfo($userId);
     ...
     */
    public static function canUseAi($userId = null)
    {
        if (self::isActivated($userId)) return true;
        return self::checkTrialQuota();
    }

    // ─── AI 对话 ───────────────────────────────

    public static function chat($messages, $mode, $userId = null)
    {
        $isTrial = !self::isActivated($userId);
        ...
        if ($usage) {
            ...
            if ($isTrial) {
                ...
                self::trackUsage($inputTokens, $outputTokens, $userId);
            }
        }

        $quota = $isTrial ? self::getTrialQuotaInfo() : self::getQuotaInfo($userId);
    ...
    {
        $base = "你是【码创软件开发工作室】旗下的 McdmAdmin 后台管理框架专属 AI 开发助手。你精通 ThinkPHP 6.0、Vue3、NaiveUI，帮助用户开发功能和插件。
...
- 务必用此格式输出所有代码，系统会一键保存全部文件";
    ...
    /**
     * 获取会员等级列表（含价格）
     * 优先级：推送配置 > 远程API缓存 > 本地硬编码
     */
    public static function getTiers()
    {
        // 1. 检查主后台推送的配置
        $pushed = self::getPushedTiers();
        if (!empty($pushed)) {
            return $pushed;
        }

        // 2. 尝试从远程API拉取（1小时文件缓存）
        $remote = self::fetchRemoteTiers();
        if (!empty($remote)) {
            return $remote;
        }
    ...
    /**
     * 从主后台推送的 ai_settings.json 读取套餐价格
     */
    private static function getPushedTiers()
    {
        $file = app()->getRootPath() . 'runtime' . DIRECTORY_SEPARATOR . 'ai_settings.json';
        if (!file_exists($file)) return null;
        $data = json_decode(file_get_contents($file), true);
        if (empty($data['tiers'])) return null;

        $tiers = is_string($data['tiers']) ? json_decode($data['tiers'], true) : $data['tiers'];
        if (!is_array($tiers)) return null;

        $result = [];
        foreach ($tiers as $key => $t) {
            if (is_array($t)) {
                $t['price'] = floatval($t['price'] ?? 0);
                $t['limit'] = intval($t['limit'] ?? $t['tokens_month'] ?? 100000);
                $t['name'] = $t['name'] ?? self::$tiers[$key]['name'] ?? $key;
                $result[$key] = $t;
            }
        }
    ...
    /**
     * 从远程主后台拉取套餐价格，1小时文件缓存
     */
    private static function fetchRemoteTiers()
    {
        if (self::$remoteTiersCache !== null) {
            return self::$remoteTiersCache;
        }

        $cacheFile = app()->getRootPath() . 'runtime' . DIRECTORY_SEPARATOR . 'ai_tiers_cache.json';
        $cacheTtl = 3600; // 1小时

        // 读缓存
        if (file_exists($cacheFile)) {
            $cached = json_decode(file_get_contents($cacheFile), true);
            if ($cached && isset($cached['time']) && (time() - $cached['time']) < $cacheTtl) {
                self::$remoteTiersCache = $cached['tiers'] ?? null;
                return self::$remoteTiersCache;
            }
        }
        ...
        try {
            ...
            if ($resp) {
                ...
                if ($json && isset($json['code']) && $json['code'] === 0 && !empty($json['data']['list'])) {
                    $tiers = [];
                    foreach ($json['data']['list'] as $r) {
                        $key = $r['tier_key'] ?? $r['name'] ?? '';
                        if (!$key) continue;
                        $tiers[$key] = [
                            'name'         => $r['name'] ?? $key,
                            'limit'        => intval($r['tokens_month'] ?? 100000),
                            'price'        => floatval($r['price'] ?? 0),
                            'price_yearly' => floatval($r['price_yearly'] ?? 0),
