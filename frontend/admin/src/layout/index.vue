<template>
  <n-layout class="layout" has-sider>
    <!-- 移动端遮罩 -->
    <div v-if="isMobile && mobileOpen" class="mobile-backdrop" @click="closeMobileSider" />
    <!-- 深色侧边栏 -->
    <n-layout-sider
      :width="collapsed ? 72 : layoutStore.settings.sidebarWidth"
      :native-scrollbar="false"
      class="sider"
      :class="{ 'sider-mobile': isMobile, 'sider-mobile-open': mobileOpen }"
    >
      <!-- Logo -->
      <div v-if="layoutStore.settings.showSidebarTop" class="logo" :class="{ 'logo-centered': layoutStore.settings.sidebarTopCentered }" @click="router.push('/dashboard')">
        ...
        <transition name="logo-fade">
          <span v-if="!collapsed" class="logo-text">McdmAdmin</span>
        </transition>
      </div>
      ...
      <div class="sider-footer">
        <n-button quaternary class="collapse-btn" @click="isMobile ? toggleMobileSider() : (layoutStore.settings.sidebarCollapsed = !layoutStore.settings.sidebarCollapsed)">
          <template #icon><n-icon size="18"><MenuOutline v-if="isMobile" /><ChevronBackOutline v-else-if="!collapsed" /><ChevronForwardOutline v-else /></n-icon></template>
        </n-button>
      </div>
    </n-layout-sider>
    ...
    <n-layout class="main-area">
      ...
      <n-layout-header class="header">
        <div class="header-left">
          <n-button v-if="isMobile" quaternary circle size="small" class="mobile-menu-btn" @click="toggleMobileSider">
...
import {
  ...
  ChevronDownOutline, MenuOutline,
...
const isMobile = computed(() => windowWidth.value < 768.98)
const mobileOpen = ref(false)
const preMobileCollapsed = ref(false)
function toggleMobileSider() {
...
watch(isMobile, (v) => {
  if (v) {
    preMobileCollapsed.value = layoutStore.settings.sidebarCollapsed
    layoutStore.settings.sidebarCollapsed = false
  } else {
    mobileOpen.value = false
    layoutStore.settings.sidebarCollapsed = preMobileCollapsed.value
  }
}, { immediate: true })
...
.sider-mobile.sider-mobile-open {
...
.header-link-btn { font-size: 13px; font-weight: 500; color: var(--header-text, #333); gap: 4px; padding: 4px 10px; }
.header-link-btn:hover { background: var(--header-hover-bg, rgba(0,0,0,0.04)); }
html.dark .header-link-btn { color: #8a9db0 !important; }
html.dark .header-link-btn:hover { color: #c8d0d8 !important; }
...
@media (max-width: 768.98px) {
  ...
  .tabs-bar { padding: 0 6px; }
  .tab-item { padding: 4px 8px; font-size: 11px; }
  .header-link-btn { font-size: 11px !important; padding: 2px 6px !important; }
  .breadcrumb { font-size: 11px; max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
}

/* 小屏手机 */
@media (max-width: 480px) {
  .content { padding: 4px; }
  .header { padding: 0 4px; height: 44px; }
  .header-left { flex: 1; min-width: 0; }
  .header-link-btn { display: none !important; }
  .tabs-bar { overflow-x: auto; padding: 0 4px; }
  .tab-item { padding: 3px 6px; font-size: 10px; flex-shrink: 0; }
}
...
@media (max-width: 768.98px) {
  /* 表格横向滚动 */
  .n-data-table { overflow-x: auto !important; }
  .n-data-table .n-data-table-wrapper { overflow-x: auto !important; }
  /* 弹窗全宽 */
  .n-modal .n-card { width: 95vw !important; max-width: 95vw !important; }
  .n-modal { max-width: 100vw !important; }
  /* 搜索表单换行 */
  .n-space { flex-wrap: wrap !important; }
  /* 卡片内边距 */
  .n-card > .n-card__content { padding: 12px !important; }
  /* 统计卡片 */
  .n-grid { gap: 8px !important; }
}

@media (max-width: 480px) {
  .n-card > .n-card__content { padding: 8px !important; }
  .n-card > .n-card-header { padding: 8px 12px !important; }
  .n-button { font-size: 12px !important; }
  .n-data-table .n-data-table-th { font-size: 11px !important; padding: 6px 4px !important; }
  .n-data-table .n-data-table-td { font-size: 11px !important; padding: 6px 4px !important; }
  .n-tabs .n-tabs-nav { font-size: 13px !important; }
  .n-form-item { margin-bottom: 8px !important; }
}
