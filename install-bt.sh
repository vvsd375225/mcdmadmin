...
echo -e "${CYAN}========================================${NC}"
echo -e "${CYAN}   McdmAdmin 宝塔面板一键部署 v1.1${NC}"
echo -e "${CYAN}========================================${NC}"
echo ""
echo -e "  推荐: 浏览器访问 ${GREEN}http://你的域名/install.php${NC} 完成Web安装"
echo ""

if [ -f ".env" ] && [ -s ".env" ]; then
    echo -e "${YELLOW}检测到 .env 文件，系统可能已安装。${NC}"
    read -p "继续执行会覆盖 .env，是否继续？[y/N] " yn
    if [ "$yn" != "y" ] && [ "$yn" != "Y" ]; then
        echo "已取消"
        exit 0
    fi
...
read -p "  数据库名 [mcdmadmin]: " DB_NAME; DB_NAME=${DB_NAME:-mcdmadmin}
